...
describe("Rover class", function() {
  ...
  it("constructor sets position and default values for mode and generatorWatts", function() {
    let roverOne = new Rover(1135);
    expect(roverOne).toEqual(new Rover(1135, 'NORMAL', 110));
    expect(roverOne.position).toEqual(1135);
  ...
  it("responds with false completed value when attempting to move in LOW_POWER mode", function() {
    let commands = [new Command('MODE_CHANGE', 'LOW_POWER'), new Command('MOVE', 500), new Command('STATUS_CHECK')];
    let message = new Message('Test message with two commands', commands);
  ...
  it("responds with position for move command", function() {
    let commands = [new Command('MODE_CHANGE', 'NORMAL'), new Command('MOVE', 500), new Command('STATUS_CHECK')];
    let message = new Message('Test message with two commands', commands);
    ...
    expect(typeof response.results[2]).toEqual('object');
    expect(response.results[1].completed).toEqual('true');
    expect(response.results[2].roverStatus.position).toEqual(500);
  });
